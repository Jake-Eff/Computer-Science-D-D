...
public class Navigator {
    ...
    private void cd(String[] args) {
        FolderNode original = currentDirectory;
        if (args[0].charAt(0) == '/') {
            currentDirectory = fileSystem.getRoot();
            args[0] = args[0].substring(1);
        }
        String[] noSlash = args[0].split("/");
        for (int i = 0; i < noSlash.length; i++) {
            if (noSlash[i].equals("..")) {
                currentDirectory = currentDirectory.getParent();
            } else if (noSlash[i].equals(".")) {

            } else {
                if (currentDirectory.getChildByName(noSlash[i]) == null) {
                    System.out.println("No such file or directory" + noSlash[i]);
                    currentDirectory = original;
                    return;
                } else {
                    if (currentDirectory.getChildByName(noSlash[i]).isFolder()) {
                        currentDirectory = (FolderNode) currentDirectory.getChildByName(noSlash[i]);
                    } else {
    ...
    private void mkdir(String[] args) {
        ...
        for (int i = 0; i < children.size(); i++) {
            if (children.get(i).getName().toLowerCase().equals(args[0].toLowerCase())) {
